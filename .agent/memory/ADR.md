...
## ADR-013: 原子化 ID 原则 (Atomic ID Principle)
*   **状态**: Accepted (2026-02-06)
*   **来源**: S04 视觉素材丢失 RCA
*   **背景**: 
    *   H5 解析器 `parse_slides.py` 采用前缀模糊匹配逻辑查找资源。
    *   当脚本定义了 `S04_A` 而资源文件夹中存在 `S04_A_Result.mp4` 时，解析器会因前缀匹配且扩展名优先级高而错误地将其作为 `S04_A` 的显示内容，导致原始素材（如 `Raw`）被覆盖丢失。
*   **决策**: 
    1.  **全局唯一原子化 ID**: 每一个展示步骤（Slide）必须拥有全局唯一的、无包含关系的 ID。
    2.  **禁止包含冲突**: 严禁一个 ID 成为另一个 ID 的起始子串（例如：禁止同时存在 `S04_A` 和 `S04_A_Result`）。
    3.  **强制显式后缀**: 对于对比类素材，必须显式标注 `_Raw` / `_Result` / `_Before` / `_After`。
*   **后果**: 
